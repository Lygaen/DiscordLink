...
public class DeafenCommand extends BaseCommand {
    ...
    private boolean deafen(Player p) {
        ...
        DiscordUtils.getMemberByPlayer(p).queue((m) -> {
            boolean deafened = m.getVoiceState().isDeafened();
            m.deafen(!deafened).queue();
            p.sendMessage("§a"+p.getName()+" ("+m.getEffectiveName()+") was "+(deafened ? "un" : "")+"deafened !");
        }, (t) -> {
